...
xinfer_imm<-subset(BLCR_classed_cellprop, select =c(Age, hannum_acc, Sex, Tumor_grade, Smoking_status, BCG, Bnv, CD4mem, CD8mem, Neu, NK, Class))
...
Cox_G2 = coxph(Surv(TenYearSurv, TenDead) ~ Age + pheno_acc + Sex + Tumor_grade + Smoking_status + BCG + Class + Group, data = GroupOS)
Cox_G3 = coxph(Surv(TenYearSurv, TenDead) ~ Age + pheno_acc + Sex + Tumor_grade + Smoking_status + BCG + Class + Group + Bnv + CD4mem + CD8mem + Neu + NK, data = GroupOS)
summary(Cox_G3)
cox.zph(Cox_G1)

# Merge Group 2~4
GroupOS_2 <- BLCR_classed_cellprop %>% 
...
Cox_G2 = coxph(Surv(TenYearSurv, TenDead) ~ Age + hannum_acc + Sex + Tumor_grade + Smoking_status + BCG + Class + Group, data = GroupOS_2)
Cox_G3 = coxph(Surv(TenYearSurv, TenDead) ~ Age + hannum_acc + Sex + Tumor_grade + Smoking_status + BCG + Class + Group + Bnv + CD4mem + CD8mem + NK, data = GroupOS_2)
summary(Cox_G3)
cox.zph(Cox_G1)
```
...
```{r}
#first subset dataset with those variables you want to include in the tree
xinfer_imm<-subset(BLCR_classed_cellprop, select =c(Age, hannum_acc, Sex, Tumor_grade, Smoking_status, BCG, Bnv, CD4mem, CD8mem, NK, Class))
OSTime = BLCR_classed_cellprop$TenYearSurv
OScen = BLCR_classed_cellprop$TenDead
model.IPCW_imm = partDSA(x=xinfer_imm, y=Surv(log(OSTime),OScen),
                     control = DSA.control(loss.function = "IPCW"))
...
Cox_G3 = coxph(Surv(TenYearSurv, TenDead) ~ Age + hannum_acc + Sex + Tumor_grade + Smoking_status + BCG + Class + Group + Bnv, data = GroupOS)
summary(Cox_G3)
```
