...
beta_for23CpGs_rR <- beta_for23CpGs[,rR_name]
beta_for23CpGs_rL <- beta_for23CpGs[,rL_name]
beta_for23CpGs_Mean_rR <- beta_for23CpGs_rR %>%
                          as.data.frame() %>%
                          mutate(rR = rowMeans(beta_for23CpGs_rR)) %>%
                          rownames_to_column(., var = "CpG") %>%
                          select(CpG,rR)
beta_for23CpGs_Mean_rL <- beta_for23CpGs_rL %>%
                          as.data.frame() %>%
                          mutate(rL = rowMeans(beta_for23CpGs_rL)) %>%
                          rownames_to_column(., var = "CpG") %>%
                          select(CpG,rL)
beta_for23CpGs_Mean <- beta_for23CpGs_Mean_rL %>%
                       left_join(beta_for23CpGs_Mean_rR) %>%
                       mutate(delta_beta = rR - rL)
beta_for23CpGs_Mean_matrix <- beta_for23CpGs_Mean_rL %>%
                              left_join(beta_for23CpGs_Mean_rR) %>%
                              column_to_rownames(., var = "CpG") %>%
                              as.matrix()
```

# Annotation for 23 CpGs
```{r}
library(minfi)
...
optimal_CpG_annotation <- optimal_CpG %>% 
                          left_join(., ann) %>%
                          left_join(., beta_for23CpGs_Mean) %>%
                          select(-AbsCoxScore, -rL, -rR)

write_csv(optimal_CpG_annotation ,"/Users/chenjiqing/Public/bladder cancer/Extended Deconvolution/Script/SS_RPMM/optimal_23CpG_annotation_pheno_acc.csv")
```
...
Top23_heat_mean <- pheatmap(
  beta_for23CpGs_Mean_matrix,
  show_rownames = F, 
  show_colnames = T,
  ...
  fontsize = 10, 
  fontsize_col = 10
  #main = "Mean Beta Values of Optimal 23 CpGs used in SS-RPMM [Whole NMIBC]"
)

save_pheatmap_png(Top23_heat_mean, "Top23_heat_mean_noRowAnnot_pheno_acc")
```
...
write_csv(common_CpG ,"/Users/chenjiqing/Public/bladder cancer/Extended Deconvolution/Manuscript/Figures and Tables/Additional File 2_pheno_acc.csv")
```
