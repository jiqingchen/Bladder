...
xinfer<-subset(phenotype3_win, select =c(Age, Sex, `Tumor grade`, `Smoking status`, muscinv, BCG, NLR))
...
#Best 4 partitions
  #Partition 1 [of 4]:
    #(muscinv is no) && (NLR <= 3.113412)
  #Partition 2 [of 4]:
    #(Smoking.status is non-smoker) && (muscinv is yes)
  #Partition 3 [of 4]:
    #(muscinv is no) && (3.113412 < NLR)
  #Partition 4 [of 4]:
    #(Smoking.status is ever-smoker) && (muscinv is yes)

GroupOS <- phenotype3_win %>% 
           mutate(Group = ifelse(muscinv == "no" & NLR <= 3.113412, "Group 1", 
                          ifelse(`Smoking status` == "non-smoker" & muscinv == "yes", 
                                 "Group 2",
                          ifelse(muscinv == "no" & NLR > 3.113412, "Group 3","Group 4"))))
table(GroupOS$Group)
...
```{r}
#first subset dataset with those variables you want to include in the tree
xinfer_imm<-subset(phenotype3_win, select =c(Age, Sex, `Tumor grade`, `Smoking status`, muscinv, BCG, Bnv, CD4mem, CD4nv, CD8mem, Neu, NK))
OSTime = phenotype3_win$TenYearSurv
OScen = phenotype3_win$TenDead
...
# Best 4 partitions
  #Partition 1 [of 4]:
    #(muscinv is no) && (CD4nv <= 1.810000) && (Neu <= 67.090000)
  #Partition 2 [of 4]:
    #(muscinv is yes)
  #Partition 3 [of 4]:
    #(muscinv is no) && (67.090000 < Neu)
  #Partition 4 [of 4]:
    #(muscinv is no) && (1.810000 < CD4nv) && (Neu <= 67.090000)

GroupOS <- phenotype3_win %>% 
           mutate(Group = ifelse(muscinv == "no" & CD4nv <= 1.81 & Neu <= 67.09, "Group 1", 
                          ifelse(muscinv == "yes","Group 2",
                          ifelse(muscinv == "no" & Neu > 67.09, "Group 3","Group 4"))))
table(GroupOS$Group)
